...
export default function AdminMaterials() {
  ...
  const MaterialCard = ({ m, showActions }: { m: Material; showActions?: boolean }) => {
    ...
    return (
      <div className="px-6 py-5 flex items-start gap-4 hover:bg-surface-container-low/30 transition-colors">
        ...
        <div className="flex-1 min-w-0">
          ...
          <p className="text-xs text-on-surface-variant/60">{new Date(m.createdAt).toLocaleDateString("vi-VN")} · {fmt(m.fileSizeBytes)} · {m.downloadCount} lượt tải</p>
        ...
        <div className="flex items-center gap-1 flex-shrink-0">
          ...
          <button onClick={()=>handleDelete(m.id)}
            className="p-2 rounded-xl hover:bg-red-50 text-on-surface-variant hover:text-red-600 transition-all">
  ...
  return (
    <main className="flex-1 p-8 max-w-6xl mx-auto w-full space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
      <section>
        <h1 className="text-3xl font-extrabold text-on-surface tracking-tight mb-1">Quản lý tài liệu</h1>
        <p className="text-on-surface-variant">Duyệt tài liệu giảng viên và quản lý tài liệu hệ thống.</p>
      </section>
      ...
      <div className="flex gap-2 border-b border-surface-container">
        {([["pending","Chờ duyệt","schedule"],["all","Tất cả","folder"],["upload","Upload hệ thống","upload"]] as const).map(([key,label,icon])=>(
          <button key={key} onClick={()=>setTab(key)}
            className={`flex items-center gap-2 px-5 py-3 text-sm font-bold border-b-2 transition-all -mb-px ${tab===key?"border-primary text-primary":"border-transparent text-on-surface-variant hover:text-on-surface"}`}>
        ...
        <section className="bg-surface-container-lowest rounded-2xl shadow-sm overflow-hidden">
          {loading ? (
            ...
            <div className="p-16 text-center">
              <span className="material-symbols-outlined text-5xl text-on-surface-variant/30 block mb-3">done_all</span>
              <p className="text-on-surface-variant font-medium">Không có tài liệu nào cần duyệt.</p>
            </div>
          ) : (
      ...
      {tab==="all" && (
        <section className="bg-surface-container-lowest rounded-2xl shadow-sm overflow-hidden">
          <div className="px-6 py-4 border-b border-surface-container flex flex-col sm:flex-row gap-4">
          ...
          {loading ? <div className="p-16 text-center"><div className="w-8 h-8 border-2 border-primary border-t-transparent rounded-full animate-spin mx-auto"/></div>
            : <div className="divide-y divide-surface-container">{filteredAll.map(m=><MaterialCard key={m.id} m={m} />)}</div>}
        </section>
      )}
        ...
        <section className="space-y-6">
          <div
            ...
            className={`border-2 border-dashed rounded-2xl p-10 text-center transition-all cursor-pointer
              ${dragging?"border-primary bg-primary/5":"border-outline-variant/40 bg-surface-container-lowest hover:border-primary/40"}`}
          ...
          >
            ...
            <h3 className="font-bold text-on-surface mb-1">Upload tài liệu hệ thống</h3>
            <p className="text-xs text-on-surface-variant">Hướng dẫn, FAQ, Quy trình · PDF, PPTX, DOCX, Video · Tối đa 50MB</p>
          </div>

          {queue.length>0&&(
            <div className="bg-surface-container-lowest rounded-2xl shadow-sm overflow-hidden">
              <div className="px-6 py-4 border-b border-surface-container flex items-center justify-between">
                <h2 className="font-bold text-on-surface">Hàng chờ ({queue.length})</h2>
                ...
                {queue.map(item=>(
                  <div key={item.key} className="p-5 space-y-3">
                    <div className="flex items-center gap-3">
                      ...
                      <button onClick={()=>setQueue(q=>q.filter(i=>i.key!==item.key))} className="material-symbols-outlined text-on-surface-variant hover:text-error text-xl">close</button>
                    </div>
                    {item.status==="idle"&&(
                      <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                        ...
                        <input value={item.subject} onChange={e=>setQueue(q=>q.map(i=>i.key===item.key?{...i,subject:e.target.value}:i))}
                          className="px-3 py-2 rounded-lg border border-outline-variant bg-surface text-sm focus:outline-none focus:border-primary" placeholder="Chủ đề..." />
                        ...
                        <input value={item.tags} onChange={e=>setQueue(q=>q.map(i=>i.key===item.key?{...i,tags:e.target.value}:i))}
                          className="px-3 py-2 rounded-lg border border-outline-variant bg-surface text-sm focus:outline-none focus:border-primary sm:col-span-3" placeholder="Thẻ (cách nhau bằng dấu phẩy)..." />
                      </div>
                    )}
      ...
      {rejectTarget && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/40 backdrop-blur-sm animate-in fade-in duration-200">
          <div className="bg-surface-container-lowest rounded-2xl shadow-xl w-full max-w-md p-6 space-y-4 animate-in zoom-in-95 duration-200">
            ...
            <textarea rows={3} value={rejectReason} onChange={e=>setRejectReason(e.target.value)}
              className="w-full px-4 py-3 rounded-xl border border-outline-variant bg-surface text-sm focus:outline-none focus:border-red-400 resize-none"
              placeholder="Vui lòng nhập lý do từ chối tài liệu này..." />
            <div className="flex gap-3">
              <button onClick={()=>setRejectTarget(null)}
                className="flex-1 py-2.5 rounded-xl border border-outline-variant text-on-surface font-bold text-sm">Hủy</button>
