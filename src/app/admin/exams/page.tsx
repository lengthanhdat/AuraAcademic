...
export default function AdminExamsPage() {
  ...
  const statusMap: any = {
    DRAFT: { label: "Bản nháp", cls: "bg-slate-500/10 text-slate-400 border-slate-500/20" },
  ...
  return (
    <div className="p-6 space-y-6 bg-[#0f172a] min-h-screen relative">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-black text-white">Quản lý bài thi</h1>
          <p className="text-slate-500 text-sm mt-1">Giám sát và quản lý tất cả bài thi trên hệ thống</p>
        </div>
        <button onClick={loadExams} className="flex items-center gap-2 px-4 py-2 bg-slate-800 border border-slate-700 text-slate-300 rounded-xl text-sm font-semibold hover:bg-slate-700 transition-all">
      ...
      <div className="bg-[#1e293b] border border-slate-700/50 rounded-2xl overflow-hidden">
        <div className="flex items-center gap-3 px-6 py-4 border-b border-slate-700/30">
          ...
          <span className="ml-auto text-xs text-slate-500">{filtered.length} bài thi</span>
        </div>
          ...
          <table className="w-full text-left">
            ...
            <tbody className="divide-y divide-slate-700/30">
              {loading ? (
                ...
                <tr><td colSpan={6} className="px-6 py-16 text-center text-slate-500">Không tìm thấy bài thi nào.</td></tr>
              ) : filtered.map(e => {
                ...
                return (
                  <tr key={e.id} className="hover:bg-slate-800/40 transition-colors group">
                    <td className="px-6 py-4">
                      <p className="font-semibold text-white text-sm max-w-[200px] truncate" title={e.title}>{e.title}</p>
                      <p className="text-xs text-slate-500 mt-1">{e.questions?.length || 0} câu hỏi</p>
                    </td>
                    ...
                    <td className="px-6 py-4 text-sm text-slate-400">{e.createdBy || "System"}</td>
