...
export default function SettingsPage() {
  ...
  const Toggle = ({ label, desc, configKey }: { label: string; desc: string; configKey: string }) => {
  ...
  const Field = ({ label, configKey, type = "text" }: { label: string; configKey: string; type?: string }) => (
            ...
            {active === "general" && (
              <>
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <Field label="Tên hệ thống" configKey="system_name" />
                  <Field label="Phiên bản" configKey="system_version" />
                </div>
                <Field label="Mô tả hệ thống" configKey="system_desc" />
              </>
            )}
            {active === "security" && (
              <div>
                <Toggle label="Yêu cầu xác thực 2 bước (2FA)" desc="Bắt buộc tất cả người dùng bật xác thực đa yếu tố" configKey="require_2fa" />
                <Toggle label="Khoá tài khoản sau 5 lần đăng nhập sai" desc="Tự động khoá và gửi email cảnh báo" configKey="lock_after_5_fails" />
                <Toggle label="Chặn đăng nhập đồng thời" desc="Chỉ cho phép 1 phiên hoạt động cùng lúc" configKey="prevent_concurrent_login" />
                <Toggle label="Ghi log toàn bộ hoạt động" desc="Audit log đầy đủ cho tất cả API calls" configKey="enable_audit_log" />
              </div>
            )}
            {active === "email" && (
              <div className="space-y-4">
                <Toggle label="Gửi email xác thực khi đăng ký" desc="Yêu cầu xác thực email trước khi đăng nhập" configKey="require_email_verify" />
                <Toggle label="Gửi cảnh báo đăng nhập lạ" desc="Thông báo khi phát hiện IP hoặc thiết bị mới" configKey="alert_suspicious_login" />
              </div>
            )}
            {active === "exam" && (
              <div>
                <Toggle label="Bật giám sát camera" desc="Kích hoạt AI giám sát trong lúc làm bài" configKey="enable_ai_proctor" />
                <Toggle label="Phát hiện gian lận tự động" desc="Tự động cảnh báo khi phát hiện hành vi nghi vấn" configKey="auto_detect_cheat" />
              </div>
            )}
            {active === "maintenance" && (
              <div className="space-y-4">
                <Toggle label="Chế độ bảo trì" desc="Tạm thời chặn tất cả truy cập từ người dùng (Chỉ Admin vào được)" configKey="maintenance_mode" />
