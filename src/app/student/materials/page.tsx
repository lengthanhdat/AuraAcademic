...
const CAT_LABELS: Record<string, string> = {
  lecture: "Bài giảng", exercise: "Bài tập", reference: "Tham khảo", guide: "Hướng dẫn", faq: "FAQ"
};
...
export default function StudentMaterials() {
  ...
  return (
    <main className="flex-1 p-8 max-w-6xl mx-auto w-full space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
      {/* Header */}
      <section>
        <h1 className="text-3xl font-extrabold text-on-surface tracking-tight mb-1">Tài liệu học</h1>
        <p className="text-on-surface-variant">Kho tài nguyên học tập từ giảng viên và nhà trường.</p>
      </section>
              ...
              return (
                <div key={m.id} className="group bg-gradient-to-br from-primary/5 to-primary/10 hover:from-primary/10 hover:to-primary/15 rounded-2xl p-6 border border-primary/10 flex gap-4 transition-all hover:shadow-md">
                  ...
                  <div className="flex flex-col gap-2 flex-shrink-0">
                    <button onClick={()=>setPreviewItem(m)} className="p-2 rounded-xl bg-surface/80 hover:bg-white text-on-surface-variant hover:text-primary transition-all" title="Xem trước">
                      <span className="material-symbols-outlined text-lg">visibility</span>
                    </button>
                    <button onClick={()=>trackDownload(m)} className="p-2 rounded-xl bg-primary/10 hover:bg-primary/20 text-primary transition-all" title="Tải xuống">
      ...
      <section className="bg-surface-container-lowest rounded-2xl p-5 shadow-sm space-y-4">
        <div className="relative">
          ...
          <input value={search} onChange={e=>setSearch(e.target.value)}
            ...
            placeholder="Tìm kiếm theo tên, nội dung, thẻ..." />
        </div>
        <div className="flex flex-wrap gap-2 items-center">
          <span className="text-xs font-bold text-on-surface-variant">Môn học:</span>
          {subjects.map(s=>(
        ...
        <div className="flex flex-wrap gap-2 items-center justify-between">
          <div className="flex gap-2 flex-wrap items-center">
            <span className="text-xs font-bold text-on-surface-variant">Loại:</span>
            {categories.map(c=>(
      ...
      <section>
        <div className="flex items-center justify-between mb-4">
          <h2 className="font-bold text-on-surface">Tất cả tài liệu</h2>
          <span className="text-xs text-on-surface-variant">{filtered.length} tài liệu</span>
        </div>
        ...
        {loading ? (
          ...
          <div className="bg-surface-container-lowest rounded-2xl p-16 text-center shadow-sm">
            ...
            <p className="text-on-surface-variant font-medium">Không tìm thấy tài liệu phù hợp.</p>
          </div>
        ) : (
                ...
                <div key={m.id} className="group bg-surface-container-lowest rounded-2xl p-5 shadow-sm hover:shadow-md border border-transparent hover:border-primary/10 transition-all hover:-translate-y-0.5 flex flex-col gap-4">
                  ...
                  <div className="pt-3 border-t border-surface-container flex items-center justify-between mt-auto">
                    <div>
                      ...
                      <p className="text-[10px] text-on-surface-variant/60">{m.fileSizeBytes?fmt(m.fileSizeBytes):""} · {m.downloadCount} tải</p>
                    </div>
                    <div className="flex gap-1">
                      <button onClick={()=>setPreviewItem(m)}
                        className="p-2 rounded-xl hover:bg-surface-container text-on-surface-variant hover:text-primary transition-all" title="Xem trước">
                        <span className="material-symbols-outlined text-lg">visibility</span>
                      </button>
                      <button onClick={()=>trackDownload(m)}
                        className="p-2 rounded-xl hover:bg-primary/10 text-on-surface-variant hover:text-primary transition-all" title="Tải xuống">
                        <span className="material-symbols-outlined text-lg">download</span>
                      </button>
                    </div>
                  </div>
            ...
            <div className="flex-1 overflow-auto p-6 space-y-4">
              ...
              {previewItem.fileUrl?.startsWith("data:application/pdf") && (
                <div className="bg-surface-container rounded-xl p-8 text-center">
                  ...
                  <p className="text-sm text-on-surface-variant mb-4">Nhấn tải xuống để xem tệp PDF trên máy tính của bạn.</p>
                </div>
              )}
              <div className="grid grid-cols-3 gap-3 pt-2">
                {[
                  { label: "Kích thước", value: previewItem.fileSizeBytes ? `${(previewItem.fileSizeBytes/1048576).toFixed(1)} MB` : "—" },
                  { label: "Lượt tải", value: `${previewItem.downloadCount}` },
                  { label: "Ngày đăng", value: new Date(previewItem.createdAt).toLocaleDateString("vi-VN") },
                ].map(({label,value})=>(
