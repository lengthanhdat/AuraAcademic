...
export default function StudentResults() {
  ...
  const getRank = (score: number) => {
    if (score >= 9.0) return { label: "Xuất sắc", cls: "bg-amber-100 text-amber-800" };
    if (score >= 8.0) return { label: "Giỏi", cls: "bg-blue-100 text-blue-800" };
    if (score >= 6.5) return { label: "Khá", cls: "bg-green-100 text-green-800" };
    if (score >= 5.0) return { label: "Trung bình", cls: "bg-slate-100 text-slate-700" };
  ...
  return (
    <main className="flex-1 p-8 max-w-6xl mx-auto w-full space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
      {/* Header */}
      <section>
        <h1 className="text-3xl font-extrabold text-on-surface tracking-tight mb-1">Kết quả thi</h1>
        <p className="text-on-surface-variant">Xem lại toàn bộ lịch sử và điểm số các kỳ thi của bạn.</p>
      </section>

      {/* Stats Cards */}
      <section className="grid grid-cols-2 md:grid-cols-4 gap-4">
        {[
          { label: "Tổng bài thi", value: totalExams.toString(), icon: "assignment", color: "text-blue-500", bg: "bg-blue-50" },
          { label: "Điểm trung bình", value: avgScore.toFixed(2), icon: "grade", color: "text-primary", bg: "bg-primary/5" },
          { label: "Tỷ lệ đạt", value: `${passRate}%`, icon: "check_circle", color: "text-emerald-500", bg: "bg-emerald-50" },
      ...
      <section className="bg-surface-container-lowest rounded-2xl p-5 shadow-sm flex flex-col sm:flex-row gap-4 items-center">
        <div className="relative flex-1 w-full">
          ...
          <input
            ...
            placeholder="Tìm theo tên kỳ thi hoặc mã đề..."
          />
        </div>
        <div className="flex gap-2 flex-wrap">
          {(["all", "pass", "fail"] as const).map(f => (
            ...
            >
              {f === "all" ? "Tất cả" : f === "pass" ? "✓ Đạt" : "✗ Không đạt"}
            </button>
          ))}
          <select
            value={sortBy}
      ...
      <section className="bg-surface-container-lowest rounded-2xl shadow-sm overflow-hidden">
        <div className="px-6 py-4 border-b border-surface-container flex items-center justify-between">
          ...
          <span className="text-xs text-on-surface-variant font-medium">{filtered.length} bài thi</span>
        </div>

        {loading ? (
          <div className="p-16 text-center">
            <div className="w-10 h-10 border-2 border-primary border-t-transparent rounded-full animate-spin mx-auto mb-4" />
            <p className="text-on-surface-variant text-sm">Đang tải kết quả...</p>
          </div>
        ) : filtered.length === 0 ? (
          <div className="p-16 text-center">
            <span className="material-symbols-outlined text-5xl text-on-surface-variant/30 block mb-4">assignment_turned_in</span>
            <p className="text-on-surface-variant font-medium">Chưa có kết quả phù hợp.</p>
            <p className="text-on-surface-variant/60 text-sm mt-1">Hãy thử thay đổi bộ lọc hoặc tham gia kỳ thi đầu tiên!</p>
          </div>
        ) : (
                ...
                <div key={r.id} className="hover:bg-surface-container-low/50 transition-colors">
                  ...
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      <p className="font-bold text-on-surface truncate">{r.examTitle || `Phòng: ${r.examId}`}</p>
                      <p className="text-xs text-on-surface-variant">
                        Mã đề: {r.versionCode} &nbsp;·&nbsp; {new Date(r.submittedAt).toLocaleDateString("vi-VN", { day: "2-digit", month: "2-digit", year: "numeric", hour: "2-digit", minute: "2-digit" })}
                      </p>
                    </div>
                  ...
                  {isExpanded && (
                    <div className="px-6 pb-5 grid grid-cols-1 sm:grid-cols-3 gap-3 animate-in fade-in slide-in-from-top-2 duration-200">
                      <div className="bg-surface-container p-4 rounded-xl">
                        <p className="text-[10px] font-bold text-on-surface-variant uppercase tracking-wider mb-1">Điểm số</p>
                        <p className="text-xl font-black text-primary">{r.score.toFixed(1)} / 10</p>
                      </div>
                      <div className="bg-surface-container p-4 rounded-xl">
                        <p className="text-[10px] font-bold text-on-surface-variant uppercase tracking-wider mb-1">Xếp loại</p>
                        <span className={`px-2 py-1 rounded-lg text-xs font-black ${rank.cls}`}>{rank.label}</span>
                      </div>
                      <div className="bg-surface-container p-4 rounded-xl">
                        <p className="text-[10px] font-bold text-on-surface-variant uppercase tracking-wider mb-1">Thời gian nộp</p>
