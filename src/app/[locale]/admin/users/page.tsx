    ...
    <div className="p-6 space-y-6 bg-[#0f172a] min-h-screen relative">
        ...
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          ...
          <div className="bg-[#1e293b] border border-slate-700 rounded-2xl w-full max-w-md p-6">
            <div className="flex items-center justify-between mb-6">
              <h3 className="font-bold text-white text-lg">Tính năng chưa khả dụng</h3>
              <button onClick={() => setShowAddModal(false)} className="text-slate-500 hover:text-white"><span className="material-symbols-outlined">close</span></button>
            </div>
            <p className="text-slate-400 text-sm mb-4">Vui lòng sử dụng tính năng Thêm người dùng ở trang Dashboard.</p>
            <button onClick={() => setShowAddModal(false)} className="w-full py-2.5 bg-slate-800 border border-slate-700 text-slate-300 rounded-xl font-semibold text-sm hover:bg-slate-700">Đóng</button>
          </div>
        </div>
      )}
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-slate-500 text-sm mt-1">Tra cứu, chỉnh sửa thông tin tài khoản và phân quyền</p>
        </div>
        <div className="flex gap-2">
          <button onClick={loadUsers} className="flex items-center gap-2 px-4 py-2 bg-slate-800 border border-slate-700 text-slate-300 rounded-xl text-sm font-semibold hover:bg-slate-700 transition-all">
