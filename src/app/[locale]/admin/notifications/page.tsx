    ...
    <div className="p-6 space-y-6 bg-[#0f172a] min-h-screen">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-slate-500 text-sm mt-1">{unread} thông báo chưa đọc</p>
        </div>
        {unread > 0 && (
          <button onClick={markAll} className="px-4 py-2 bg-slate-800 border border-slate-700 text-slate-300 rounded-xl text-sm font-semibold hover:bg-slate-700 transition-all">
        ...
        {notifs.map(n => {
          ...
          return (
            <div key={n.id} onClick={() => markRead(n.id)}
              className={`flex items-start gap-4 p-5 bg-[#1e293b] border rounded-2xl cursor-pointer transition-all hover:border-slate-600 ${n.read ? "border-slate-700/30 opacity-60" : "border-slate-700/50"}`}>
              ...
              <button onClick={e => { e.stopPropagation(); dismiss(n.id); }} className="text-slate-600 hover:text-slate-400 transition-colors flex-shrink-0">
