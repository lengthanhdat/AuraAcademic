...
export default function TeacherProfile() {
  ...
  const getExamStatus = (exam: any) => {
    if (exam.status === 'FINISHED') return { label: 'Đã kết thúc', color: 'bg-slate-100 text-slate-600' };
    if (exam.status === 'DRAFT') return { label: 'Bản nháp', color: 'bg-blue-100 text-blue-800' };
    
    if (exam.status === 'PUBLISHED') {
      if (exam.startTime && exam.duration) {
        const endTime = exam.startTime + (exam.duration * 60 * 1000);
        if (Date.now() > endTime) {
          return { label: 'Đã kết thúc', color: 'bg-slate-100 text-slate-600' };
        }
      }
        ...
        <div className="bg-surface-container-lowest rounded-xl p-8 shadow-sm relative overflow-hidden flex flex-col md:flex-row items-center gap-8">
          ...
          <div className="flex gap-3">
            ...
            >
              ...
              {isEditing ? "Hủy cập nhật" : "Cập nhật hồ sơ"}
            </button>
          ...
          <form onSubmit={handleSaveProfile} className="space-y-4">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div>
                ...
                <input value={form.title} onChange={e => setForm({ ...form, title: e.target.value })} className="w-full px-4 py-3 rounded-xl border border-outline-variant bg-surface focus:outline-none focus:border-primary text-on-surface transition-colors" placeholder="VD: Giảng viên Cao cấp" />
              </div>
              <div>
