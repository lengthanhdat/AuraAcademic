              ...
              <div key={item.key} className="p-5 space-y-3">
                ...
                {item.status === "error" && (() => {
                  ...
                  const violationIcons: Record<string, { icon: string; label: string }> = {
                    PROFANITY:       { icon: "no_adult_content", label: "Ngôn từ không phù hợp" },
                    SEXUAL_CONTENT:  { icon: "block",            label: "Nội dung tình dục" },
                    VIOLENCE:        { icon: "warning",          label: "Bạo lực" },
                    HATE_SPEECH:     { icon: "report",           label: "Phát ngôn thù ghét" },
                    POLITICAL:       { icon: "gavel",            label: "Nhạy cảm chính trị" },
                    COPYRIGHT:       { icon: "copyright",        label: "Vi phạm bản quyền" },
                  };
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-2 mb-0.5 flex-wrap">
                          <p className="text-xs font-black text-red-700 uppercase tracking-wide">
                            {isAiReject ? "AI Kiểm Duyệt Từ Chối" : "Lỗi Upload"}
                          </p>
                          {vi && (
                  ...
                  <div className="space-y-1.5">
                    <div className="flex items-center justify-between">
                      <span className="text-[10px] font-bold text-on-surface-variant flex items-center gap-1">
                        ...
                        {item.progress < 40 ? "Đang tải file..." : "AI đang kiểm duyệt nội dung..."}
                      </span>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/40 backdrop-blur-sm animate-in fade-in duration-200">
          <div className="bg-surface-container-lowest rounded-2xl shadow-xl w-full max-w-lg p-6 space-y-4 animate-in zoom-in-95 duration-200">
            ...
            {[
              ...
              { field: "subject", label: "Môn học", type: "text" },
              { field: "description", label: "Mô tả", type: "text" },
              { field: "tags", label: "Thẻ (cách bằng dấu phẩy)", type: "text" },
            ].map(({ field, label }) => (
              <div key={field}>
