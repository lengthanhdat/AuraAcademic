...
export default function MonitoringPage() {
  ...
  const fetchExams = async (teacherId: string) => {
    try {
      ...
      console.error("Loi khi lay danh sach ky thi", e);
    } finally {
      setLoading(false);
    }
  };
      ...
      <div className="max-w-[1200px] mx-auto">
        <div className="flex items-center justify-between mb-8">
          <div>
            ...
            <p className="text-slate-500 mt-1 font-medium">Theo dõi hoạt động của học sinh trong các phòng thi đang mở.</p>
          </div>
        </div>
        ...
        {loading ? (
          <div className="py-20 text-center">
            ...
            <p className="mt-4 text-slate-500 font-medium">Đang tìm các phòng thi đang diễn ra...</p>
          </div>
        ) : exams.length === 0 ? (
          <div className="bg-white rounded-3xl p-16 text-center border border-slate-200 shadow-sm">
            ...
            <h2 className="text-xl font-bold text-slate-800 mb-2">Không có phòng thi nào đang mở</h2>
            <p className="text-slate-500 max-w-sm mx-auto">Các kỳ thi bạn đã công bố nhưng chưa bắt đầu hoặc chưa kết thúc sẽ hiển thị ở đây.</p>
            <button 
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {exams.map((exam) => (
              <div key={exam.id} className="bg-white rounded-2xl border border-slate-200 overflow-hidden shadow-sm hover:shadow-md transition-all group">
                <div className="p-6">
                  <div className="flex justify-between items-start mb-4">
                    <div className="flex flex-col">
                      <span className="text-[10px] font-black text-blue-600 uppercase tracking-widest mb-1">Mã phòng: {exam.accessCode}</span>
                  ...
                  <div className="grid grid-cols-2 gap-4 mb-6">
                    <div className="bg-slate-50 p-3 rounded-xl border border-slate-100">
                      <p className="text-[10px] font-bold text-slate-400 uppercase">Đang chờ</p>
                      <p className="text-xl font-black text-amber-600">{activeCountMap[exam.accessCode]?.lobbyCount || 0}</p>
                    </div>
                    <div className="bg-slate-50 p-3 rounded-xl border border-slate-100">
                      <p className="text-[10px] font-bold text-slate-400 uppercase">Đang làm bài</p>
                  ...
                  <button
                    ...
                    className="w-full py-3.5 bg-slate-800 text-white font-bold rounded-xl flex items-center justify-center gap-2 hover:bg-blue-600 transition-all"
                  >
